...
const notificationChannels = ({ strapi }: { strapi: Core.Strapi }) => ({
  ...
  async getSupportedIntegrations(ctx) {
    const supportedIntegrationsData = await service({ strapi }).makeBackendRequest(`/integrations/supported`, {
      method: 'GET',
    });
  ...
  async testNotificationChannel(ctx) {
    const { id } = ctx.params;
    const testResult = await service({ strapi }).makeBackendRequest(`/user/integrations/${id}/test`, {
      method: 'POST',
    });
    ctx.body = { testResult };
  },
  ...
  async updateNotificationChannel(ctx) {
    const { id } = ctx.params;
    const { ...data } = ctx.request.body;
    const updatedChannel = await service({ strapi }).makeBackendRequest(`/user/integrations/${id}`, {
      method: 'PUT',
      body: JSON.stringify(data),
    });
