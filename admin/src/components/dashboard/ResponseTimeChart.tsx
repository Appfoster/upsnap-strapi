...
export const ResponseTimeChart = ({
  ...
  const stats = useMemo(() => {
    if (
      ...
      Object.keys(regionResponseTimeData).length === 0
    ) {
  ...
  return (
    <Card>
      <CardHeader display="flex" style={{ justifyContent: 'space-between' }}>
        <Typography variant="delta" marginLeft={2} padding={3}>Response Time</Typography>
      ...
      <CardBody>
        <CardContent style={{ width: '650px' }}>
          ...
          <Box
            marginTop={6}
            display="flex"
            style={{ gap: 16, marginTop: 32, flexDirection: 'row' }}
          >
            <Box display='flex' style={{flexDirection: 'column', alignItems: 'center'}} background="neutral0" padding={4} borderColor={'alternative200'} borderRadius={2} hasRadius>
