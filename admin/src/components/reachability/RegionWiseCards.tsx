          ...
          return (
            <Box key={region.id} style={{ marginBottom: 16, borderRadius: 8, padding: 12 }}>
              <Box style={{ display: "flex", alignItems: "center", marginBottom: 8 }}>
                ...
                {isPrimary(region) && (
                  <Badge size={'S'} backgroundColor="warning600" textColor="neutral0" style={{ marginLeft: "auto" }}>
                    primary
                  </Badge>
                )}
              </Box>
              {isLoading ? (
                ...
                <Box display="flex" style={{ flexDirection: 'row', justifyContent: 'space-around' }}>
                  ...
                  <Box display="flex" style={{ flexDirection: 'column', alignItems: 'center' }}>
                    ...
                    <Typography variant="epsilon">{formatTime(data.max_response_time)}</Typography>
                  </Box>
                  <Box display="flex" style={{ flexDirection: 'column', alignItems: 'center' }}>
