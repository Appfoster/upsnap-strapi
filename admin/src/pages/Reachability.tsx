...
export default function Reachability() {
  const monitorId  = '9bd97575-21ae-4d59-9dc9-e9b31b4dea92' // useParams<{ monitorId: string }>();
  ...
  const [regionResponseTimeData, setRegionResponseTimeData] = useState<Record<string, RegionResponseTimeData>>({});
 ...
 console.log('data ', data);
  ...
  useEffect(() => {
    const fetchAll = async () => {
      const regions = Array.isArray(selectedMonitor?.monitor?.regions) ? selectedMonitor?.monitor?.regions : [];
  ...
  }, [selectedMonitor?.monitor?.id, selectedMonitor?.monitor?.regions, timeRange]);
